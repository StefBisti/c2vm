...
/* One row of the attribution table: a package and the findings it brought. */
struct attribution
{
    char package[160];
    char eco[16];
...
/*
 * A finding is the same finding if it is the same CVE against the same
 * package. The version is deliberately excluded: a package upgraded during
 * the conversion still carries the same vulnerability, and reporting that as
 * one gone plus one new would hide it.
 */
...
/* Most findings first; Critical breaks a tie, so the table leads with risk. */
...
/* [severity][0] = all ecosystems, [severity][1] = pkg:deb only. */
...
/*
 * Which packages introduced the new findings. "1837 new High" is a number;
 * "1778 of them are the kernel" is a result, and the difference between the
 * two is this table.
 */
static size_t attribute(const struct vuln *v, size_t n,
                        struct attribution **out)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        for (size_t j = 0; j < nrows; j++)
            if (!strcmp(rows[j].package, v[i].package) &&
                !strcmp(rows[j].eco, v[i].eco))
                row = &rows[j];
                           ...
                           const struct attribution *attr, size_t nattr)
{
    ...
    /*
     * Two columns, because they answer different questions. The all-ecosystem
     * count includes an NVD CPE match against the kernel, which reports every
     * kernel CVE regardless of the fixes the distribution backported. The deb
     * column uses the distribution's own security data. Neither is wrong; the
    ...
    for (size_t i = 0; i < nattr && i < TOP_PACKAGES; i++)
        fprintf(f, "| `%s` | %s | %zu | %zu | %zu |\n",
                attr[i].package, attr[i].eco, attr[i].count,
                attr[i].critical, attr[i].high);
    if (nattr > TOP_PACKAGES)
        fprintf(f, "\n%zu further packages contributed fewer findings each.\n",
                nattr - TOP_PACKAGES);

    /* The full list would be thousands of rows; the severe ones are the
       ones a reader needs to see in prose. */
...
int cmd_cve(int argc, char *argv[])
{
    ...
    /* One pass, unlike the SBOM diff: there is no version-change case to
       recover, because the key already ignores the version. */
